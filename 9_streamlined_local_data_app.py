        ...
        if company_options:
            ...
            if selected_company:
                ...
                if st.button("Analyze Company", use_container_width=True):
                    if company_data:
                        with st.spinner(f"Analyzing {selected_company} data with ChatGPT..."):
                            try:
                                # Set up OpenAI client using session state API key
                                client = openai.OpenAI(api_key=st.session_state['openai_api_key'])
                                ...
                                # Check if the JSON is too large for the API
                                if len(company_json) > 100000:  # ~100KB limit
                                    st.warning("Company data is very large. Sending a summary instead of full data.")
                                    # Create a summary for very large datasets
                                    summary_data = {
                                        "company_name": company_data.get("company_name", selected_company),
                                        "data_structure": {
                                            key: f"{type(value).__name__} with {len(value) if hasattr(value, '__len__') else 'N/A'} items"
                                            for key, value in company_data.items()
                                        }
                                    }
                                    
                                    # Add sample of associated_persons if it exists
                                    if 'associated_persons' in company_data and isinstance(company_data['associated_persons'], list):
                                        if len(company_data['associated_persons']) > 0:
                                            summary_data['sample_associated_persons'] = company_data['associated_persons'][:3]
                                    
                                    company_json = json.dumps(summary_data, indent=2)
                                
                                # Analysis query
                                query = f"""Please analyze this company data for {selected_company}. Provide insights about:

1. Company Overview: What type of organization this appears to be based on the name and structure
2. Associated People: Summary of the individuals connected to this company (roles, relationships, patterns)
3. Data Structure: How the information is organized (association_map, associated_persons, etc.)
4. Key Insights: Notable patterns, geographic distribution, role diversity, or interesting findings
5. Network Analysis: If possible, insights about the relationships and connections within this organization

Here is the company JSON data:
...
""" + company_json
                                ...
                                st.markdown("### ChatGPT Company Analysis Results:")
